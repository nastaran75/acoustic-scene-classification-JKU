...
def train(dataset_name,fold,division,augment,patience,representation,batch_size,epochs,learning_rate):
	...
	def iterate(name,data,label,num_replicates,batch_size,shuffle=False):
		...
		train_acc = 0.0
		train_batches = 0
		visit = np.zeros(data.shape[0]/num_replicates)
		train_pred = np.zeros([data.shape[0]/num_replicates,num_classes])
		train_voting_pred = np.zeros([data.shape[0]/num_replicates,num_classes])
		train_correct_pred = np.zeros(data.shape[0]/num_replicates)

		batches = PD.iterate_minibatches(data, label, batch_size,
		shuffle=shuffle)
		...
		for batch_cnt,batch in enumerate(batches):
			...
			for i in range(len(targets)):
				...
				train_correct_pred[indices[i]/num_replicates] = targets[i]
		...
		return float(train_err/ train_batches),float(real_train_accuracy*100)
	...
	print date
	print "Loading data..."
	X_train, y_train, X_val, y_val, X_test,y_test, num_replicates = PD.load_dataset(dataset_name,fold,division,representation,False)
	...
	for epoch in range(epochs):
		...
		val_err, val_acc = iterate('validation', X_val,y_val,num_replicates,batch_size,shuffle=True)
		test_err, test_acc = iterate('test', X_test,y_test,num_replicates,batch_size,shuffle=True)
