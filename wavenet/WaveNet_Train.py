import WaveNet_PrepareDataset as PD
import theano
import lasagne
import theano.tensor as T
from theano import shared
import WaveNet_Network as Network
from time import gmtime, strftime
import os
import numpy as np
import sys
# import progress
import WaveNet_background as bg
import WaveNet_parser as parser
...
def train(dataset_name,fold,division,augment,patience,representation,batch_size,epochs,learning_rate):
	...
	model_name = 'models/' + date + '_' + dataset_name + '_wave_model_' + str(fold) + '_' + str(division) + '_' + str(patience) + '_' + representation + '_' + str(augment) + '.npz'
	# epochs = 150
	# batch_size = 16
	num_classes = 15
	print date
	...
	prediction = sample_from_softmax(prediction)
	...
	if num_replicates != division:
		print '--------number of replicates does not match division--------'
