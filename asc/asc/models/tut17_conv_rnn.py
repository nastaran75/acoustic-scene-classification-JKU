...
from lasagne.layers import DropoutLayer, FlattenLayer, ReshapeLayer, LSTMLayer, DenseLayer, ElemwiseSumLayer, FeaturePoolLayer, GRULayer
...
from asc.utils.data_tut17 import SPEC_BINS
...
def build_model(batch_size=BATCH_SIZE):
    ...
    net_fwd = LSTMLayer(net, num_units=128, only_return_final=False,
                        ingate=in_gate, forgetgate=forget_gate, outgate=out_gate,
                        cell_init=cell_init, hid_init=hid_init)
    net_bwd = LSTMLayer(net, num_units=128, only_return_final=False, backwards=True,
                        ingate=in_gate, forgetgate=forget_gate, outgate=out_gate,
                        cell_init=cell_init, hid_init=hid_init)
    net = ElemwiseSumLayer((net_fwd, net_bwd))

    # net = GRULayer(net, num_units=16, only_return_final=False, learn_init=True)
    # net = GRULayer(net, num_units=16, only_return_final=False, learn_init=True)
    # net = FeaturePoolLayer(net, axis=1, pool_size=N_STEPS, pool_function=T.mean)

    net = DenseLayer(net, num_units=15, nonlinearity=lasagne.nonlinearities.softmax)

    return net


refinement_strategy = RefinementStrategy(n_refinement_steps=0, refinement_patience=PATIENCE, learn_rate_multiplier=0.5)
